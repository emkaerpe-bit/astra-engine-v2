  ...
  try {
    ...
    const planetResults = PLANETS.map(p => {
      ...
      if (zodiacType === 'iau') {
        ...
        if (constellation) {
          // Store actual IAU name in metadata or override sign for display
          p.iauConstellation = constellation.name;
        }
      }

      // Handle Virtual Aries Point
      if (p.key === 'ap') {
        return {
          ...p,
          longitude: 0,
          ...
          isRetrograde: false,
          ...formatDegree(0),
          dwadasamsa: calculateDwadasamsa(0),
          house: 0
        };
      }
    ...
    for (let j = i + 1; j < planets.length; j++) {
      ...
      for (const asp of DEFS) {
        if (Math.abs(diff - asp.angle) <= asp.orb) {
          aspects.push({
            planet1: { key: planets[i].key, symbol: planets[i].symbol },
            planet2: { key: planets[j].key, symbol: planets[j].symbol },
            type: asp.name, symbol: asp.symbol, orb: parseFloat(Math.abs(diff - asp.angle).toFixed(2))
          });
        }
      }
    }
