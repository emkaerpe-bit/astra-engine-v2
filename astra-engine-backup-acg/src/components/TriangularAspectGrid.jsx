...
const TriangularAspectGrid = ({ planets, title, colorClass = "text-[#1F2226]", onHover, onLeave, patterns, activePatterns }) => {
  if (!planets) return null;

  // 1. Map active patterns for easy lookup
  const activePatternAspects = React.useMemo(() => {
    const set = new Set();
    if (!patterns || !activePatterns) return set;
    
    [...activePatterns].forEach(pKey => {
      const idx = parseInt(pKey.split('-').pop());
      const pattern = patterns[idx];
      if (pattern) {
        // Add all possible pairs within the pattern
        for (let i = 0; i < pattern.planets.length; i++) {
          for (let j = i + 1; j < pattern.planets.length; j++) {
            const p1 = pattern.planets[i];
            const p2 = pattern.planets[j];
            set.add(`${p1}-${p2}`);
            set.add(`${p2}-${p1}`);
          }
        }
          ...
          {gridPlanets.map((p1, rowIndex) => (
            <div key={`row-${p1.key}`} className="flex w-full h-8">
              ...
              {gridPlanets.map((p2, colIndex) => {
                ...
                const isPartOfPattern = activePatternAspects.has(`${p1.key}-${p2.key}`);

                return (
                  <div 
                    key={`cell-${p1.key}-${p2.key}`} 
                    className={`flex-1 border-r border-b border-[#C9BEB1]/10 flex flex-col items-center justify-center relative group hover:bg-[#D4AF37]/10 transition-colors cursor-help ${isPartOfPattern ? 'bg-[#D4AF37]/10' : 'bg-white'}`}
                    onMouseEnter={(e) => {
                      if (aspect) {
                        const typeLabel = aspect.isApplying ? 'aplikacyjny' : 'separacyjny';
                        const patternInfo = isPartOfPattern ? ' [Część Aktywnej Figury]' : '';
                        onHover?.(e, `${t(p1.name || p1.key)} ${aspect.name} ${t(p2.name || p2.key)} (${aspect.orb.toFixed(1)}° - ${typeLabel})${patternInfo}`);
                      }
                    }}
                    onMouseLeave={onLeave}
                  >
                    {isPartOfPattern && (
                      <div className="absolute inset-0 border border-[#D4AF37] pointer-events-none opacity-40" />
                    )}
                    {aspect && (
                      <>
