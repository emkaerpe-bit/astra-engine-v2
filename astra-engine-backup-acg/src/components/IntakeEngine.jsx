...
import { MapPin, Clock, CalendarDays, Settings, Compass } from 'lucide-react';

export default function IntakeEngine({ onSubmit }) {
  const formRef = useRef(null);
  const mandalRef = useRef(null);
  const ctxRef = useRef(null);
  const [zodiacType, setZodiacType] = useState('tropical');
  ...
  const handleSubmit = (e) => {
    ...
    const values = {
      ...
      zodiacType: fd.get('zodiacType'),
      ayanamsa: fd.get('ayanamsa') || 1,
    };
            ...
            {Array.from({ length: 12 }, (_, j) => {
              ...
              return (
                <line key={j}
                  ...
                  x2={400 + r * Math.cos(angle)} y2={400 + r * Math.cos(angle)}
      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <form onSubmit={handleSubmit}
          ...
          <div className="grid grid-cols-2 gap-4 mb-5">
            <div>
              <label className="flex items-center gap-2 mb-2 text-[10px] font-sans font-bold tracking-widest uppercase text-[#1F2226]">
                <Compass size={12} className="text-[#8C8883]" /> Zodiak
              </label>
              <select 
                name="zodiacType" 
                className="astra-input bg-white appearance-none cursor-pointer text-xs"
                value={zodiacType}
                onChange={(e) => setZodiacType(e.target.value)}
              >
                <option value="tropical">Tropikalny</option>
                <option value="sidereal">Syderyczny</option>
              </select>
            </div>
            <div>
              <label className="flex items-center gap-2 mb-2 text-[10px] font-sans font-bold tracking-widest uppercase text-[#1F2226]">
                <Settings size={12} className="text-[#8C8883]" /> Domy
              </label>
              <select name="houseSystem" className="astra-input bg-white appearance-none cursor-pointer text-xs">
          ...
          {zodiacType === 'sidereal' && (
            <div className="mb-8 animate-in fade-in slide-in-from-top-2 duration-300">
              <label className="flex items-center gap-2 mb-2 text-[10px] font-sans font-bold tracking-widest uppercase text-[#D4AF37]">
                Ajanamsza (Sidereal Mode)
              </label>
              <select name="ayanamsa" className="astra-input bg-white appearance-none cursor-pointer text-xs border-[#D4AF37]/30">
          ...
          <button type="submit"
            className="btn-magnetic w-full py-4 rounded-2xl font-sans font-bold tracking-widest uppercase text-sm shadow-[0_4px_20px_rgba(212,175,55,0.2)] mt-4"
