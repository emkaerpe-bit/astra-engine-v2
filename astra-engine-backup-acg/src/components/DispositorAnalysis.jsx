    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          {finalDispositors.length > 0 && (
            <div className="bg-[#1F2226] p-4 rounded-2xl border border-[#D4AF37]/50 shadow-xl overflow-hidden relative group">
              ...
              <div className="flex justify-between items-start mb-3">
                <h4 className="text-[#D4AF37] text-[10px] font-mono uppercase tracking-[0.3em] font-bold">Władca Finalny</h4>
                <div className="px-2 py-0.5 rounded-full bg-[#D4AF37]/20 border border-[#D4AF37]/40 text-[7px] text-[#D4AF37] font-mono font-bold uppercase tracking-widest">
                  Instancja Ostateczna
                </div>
              </div>
              <div className="flex flex-wrap gap-3">
                {finalDispositors.map(pk => {
                  const data = summary[pk] || { percentage: 0 };
                  return (
                    <div key={pk} className="flex flex-col gap-1">
                      <div className="flex items-center gap-3 bg-white/10 px-4 py-2 rounded-xl border border-white/10 hover:border-[#D4AF37] transition-all">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[7px] font-mono text-[#D4AF37] uppercase font-bold">Wpływ: {data.percentage.toFixed(1)}%</span>
                        </div>
                      </div>
                    </div>
                  );
              ...
              <p className="text-[8px] text-white/50 mt-4 italic font-mono uppercase leading-relaxed">
                Planeta "u siebie", do której prowadzą wszystkie lub wybrane łańcuchy. Ma ostatnie słowo w tematach, które rządzona przez nią grupa planet podejmuje.
              </p>
            </div>
          )}
          ...
          {/* 1.2 ATMAKARAKA - SOUL PLANET */}
          {(() => {
            const ak = [...planets]
              .filter(p => !['node', 'south_node', 'lilith', 'fortune', 'spirit', 'eros', 'chiron'].includes(p.key))
              .sort((a, b) => (b.longitude % 30) - (a.longitude % 30))[0];
            
            if (!ak) return null;
            
            return (
              <div className="bg-[#FBF7F1] p-4 rounded-2xl border border-[#D4AF37]/30 shadow-lg relative overflow-hidden group">
                <div className="absolute top-0 right-0 p-4 opacity-5 group-hover:opacity-10 transition-opacity">
                   <PlanetGlyph name={ak.key} size={50} />
                </div>
                <h4 className="text-[#D4AF37] text-[10px] font-mono uppercase tracking-[0.3em] font-bold mb-3">Atmakaraka (Soul Planet)</h4>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-[#1F2226] flex items-center justify-center text-[#D4AF37]">
                    <PlanetGlyph name={ak.key} size={20} />
                  </div>
                  <div>
                    <span className="block text-lg font-serif italic font-bold text-[#1F2226] leading-none">
                      {t(ak.name)}
                    </span>
                    <span className="text-[10px] font-mono text-[#D4AF37] font-bold mt-1 block">
                      {ak.degreeInSign} — Najwyższy Stopień
                    </span>
                  </div>
                </div>
                <p className="text-[8px] text-[#8C8883] mt-3 italic font-mono uppercase leading-relaxed">
                  Planeta reprezentująca esencję duszy i główne wyzwanie wcielenia.
                </p>
              </div>
            );
          })()}
